...
function PeerServer(options) {
  ...
  this._options = util.extend({
    ...
    ssl: {},
    mongo: 'mongodb://localhost:27017/peerjs'
  }, options);
  ...
  this._wss.on('connection', function(socket) {
    ...
    if (!self._clients[key] || !self._clients[key][id]) {
      self._checkKey(key, ip, function(err) {
        if (!err) {
          if (self._addPeer(key, id, token, ip)) {
...
PeerServer.prototype._addPeer = function(key, id, token, ip) {
  var fresh = false;

  if (!this._clients[key][id]) {
    this._clients[key][id] = { token: token, ip: ip };
    this._ips[ip]++;
